...
from entity_management_async.atlas import CellComposition
from entity_management_async.config import BrainRegionSelectorConfig, MacroConnectomeConfig
from entity_management_async.nexus import download_file, file_as_dict, load_by_id
...
async def download_and_get_ids_from_distribution(entity, path, mapping):
    ...
    for d_item in distribution:
        ...
        if url:
            ...
            print(f"    {filename}")
            await download_file(url, path, file_name=filename)
            mapping[url] = filename
            if d_item.get("encodingFormat", "") == "application/json":
                content = await file_as_dict(url)
                ids.update(_get_ids_from_dict(content))

    return ids


async def _download_entity_get_ids(id_, path, mapping):
    def _err_exit():
        print(f"\nERROR: Can't fetch: {id_}\n")
        return set()

    try:
        entity = await load_by_id(id_)
    ...
    ids.update(await download_and_get_ids_from_distribution(entity, path, mapping))

    return ids


async def _download_entity_recursive(id_, path, depth, mapping=None):
    mapping = mapping or {}

    if id_ in mapping:
        return mapping

    entity_ids = await _download_entity_get_ids(id_, path, mapping)

    if depth > 0:
        for sub_id in entity_ids:
            await _download_entity_recursive(sub_id, path, depth - 1, mapping)

    return mapping


async def download_model_config(model_config, path, depth):
    """Download model config recursively."""
    os.makedirs(path, exist_ok=True)
    print(f"\nSaving files to '{path}':\n")
    mapping = await _download_entity_recursive(model_config.get_id(), path, depth)
