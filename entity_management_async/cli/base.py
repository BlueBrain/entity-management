...
from entity_management_async.cli.model_building_config import (
    download_model_config,
    model_building_config_as_dict,
)
from entity_management_async.config import ModelBuildingConfig
from entity_management_async.nexus import load_by_id, load_by_url
from entity_management_async.util import run_maybe_async, split_url_params
...
@run_maybe_async
async def _get(id_or_url, output, max_depth):
    ...
    data = await load_by_url(id_or_url) or await load_by_id(id_or_url, cross_bucket=True)
    if not data:
    ...
    if "ModelBuildingConfig" in types:
        config = await ModelBuildingConfig.from_id(data["@id"], cross_bucket=True)
        pprint(model_building_config_as_dict(config))

        if output is not None:
            await download_model_config(config, output, max_depth)
    else:
