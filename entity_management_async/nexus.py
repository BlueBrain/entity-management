...
from entity_management_async import httpw
from entity_management_async.debug import PP
from entity_management_async.settings import DASH, JSLD_TYPE, NSG, SCHEMA_UNCONSTRAINED, USERINFO
...
from entity_management_async.util import quote, split_url_params, unquote_uri_path
    ...
    async def wrapper(*args, **kwargs):
        ...
        try:
            ...
            if (
                ...
                kwargs["token"] = await refresh_token()
                try:
                    return await func(*args, **kwargs)
...
@_nexus_wrapper
async def get_type_from_id(
    resource_id, base=None, org=None, proj=None, token=None, cross_bucket=False
):
    ...
    response = await httpw.get(url, headers=_get_headers(token), timeout=10)
...
@_nexus_wrapper
async def create(base_url, payload, resource_id=None, sync_index=False, token=None):
...
@_nexus_wrapper
async def update(id_url, rev, payload, sync_index=False, token=None):
...
@_nexus_wrapper
async def deprecate(id_url, rev, sync_index=False, token=None):
    ...
    if sync_index:
        params.update({"indexing": "sync"})
    response = await httpw.delete(id_url, headers=_get_headers(token), params=params, timeout=10)
    response.raise_for_status()
    return _to_json(response)


@_nexus_wrapper
async def load_by_url(url, params=None, stream=False, token=None):
    ...
    """
    response = await httpw.get(url, headers=_get_headers(token), params=params, timeout=10)
...
async def load_by_id(
    ...
    url = f"{base_url}/{quote(resource_id)}"
    return await load_by_url(url=url, params=params, stream=stream, token=token)


@_nexus_wrapper
async def get_current_agent(token=None):
    """Get user info"""
    if token is None:
        return None

    response = await httpw.get(
        USERINFO,
...
@_nexus_wrapper
async def _get_file_metadata(url, tag=None, token=None):
    """Helper function"""
    response = await httpw.get(
...
async def get_file_rev(url, tag=None, token=None):
    """Get file rev.
    ...
    Returns:
        File revision.
    """
    metadata = await _get_file_metadata(url, tag=tag, token=token)
    return metadata["_rev"]


async def get_file_location(url, tag=None, token=None):
    """Get file location.
...
async def get_unquoted_uri_path(url, tag=None, token=None):
    ...
    Args:
        ...
        token (str): Optional OAuth token.
    """
    location = await get_file_location(url, tag=tag, token=token)
    return unquote_uri_path(location)


async def get_file_name(url, tag=None, token=None):
    """Get file rev.
...
@_nexus_wrapper
async def download_file(url, path, file_name=None, tag=None, rev=None, token=None):
    """Download file.
...
@_nexus_wrapper
async def file_as_dict(url, tag=None, rev=None, token=None):
    """Stream file.
    ...
    async with httpw.stream(
        ...
        return js.loads(await response.aread())


@_nexus_wrapper
async def sparql_query(query, base=None, org=None, proj=None, token=None, operation="query"):
...
@_nexus_wrapper
async def es_query(query, base=None, org=None, proj=None, token=None):
