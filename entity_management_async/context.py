...
async def get_resolved_context(obj, *, base=None, org=None, proj=None, token=None):
    ...
    """
    document = await _resolve_context(
        obj, visited=set(), base=base, org=org, proj=proj, token=token
    )
    return Context(document, version=1.1)


async def _load_context(resource_id, base=None, org=None, proj=None, token=None):

    if resource_id in _CONTEXT_CACHE:
        return _CONTEXT_CACHE[resource_id]

    data = await nexus.load_by_id(
...
async def _resolve_context(context, visited, base=None, org=None, proj=None, token=None):
    ...
    if isinstance(context, dict):
        ...
        visited.add(context)
        result = await _load_context(context, base=base, org=org, proj=proj, token=token)
        result = await _resolve_context(
