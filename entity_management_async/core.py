...
from entity_management_async.base import Subject  # noqa pylint: disable=unused-import
from entity_management_async.base import BlankNode, Identifiable, _NexusBySparqlIterator, attributes
from entity_management_async.settings import WORKFLOW
from entity_management_async.typing import MaybeList
from entity_management_async.util import AttrOf, NotInstantiated, unquote_uri_path
...
class DataDownload(BlankNode):
    ...
    async def download(self, path=None, file_name=None, use_auth=None):
        """Download ``contentUrl``.
        ...
        if path is None:
            path = os.getcwd()
        return await nexus.download_file(self.contentUrl, path, file_name, token=use_auth)

    async def get_location(self, use_auth=None):
        ...
        # pylint: disable=no-member
        assert self.contentUrl is not None, "No contentUrl!"
        return await nexus.get_file_location(self.contentUrl, token=use_auth)

    async def get_location_path(self, use_auth=None):
        ...
        # pylint: disable=no-member
        assert self.contentUrl is not None, "No contentUrl!"
        return await nexus.get_unquoted_uri_path(self.contentUrl, token=use_auth)
    ...
    async def as_dict(self, use_auth=None):
        ...
        return await nexus.file_as_dict(self.contentUrl, token=use_auth)
    ...
    async def publish(
        ...
        *,
        ...
        if self.wasInfluencedBy is None and WORKFLOW is not None:  # pylint: disable=no-member
            ...
            workflow = await WorkflowExecution.from_id(
    ...
    async def publish(
        ...
        *,
        ...
        if self.wasGeneratedBy is None and activity is None and WORKFLOW is not None:
            # in case running in the context of workflow execution activity
            activity = await WorkflowExecution.from_id(
                WORKFLOW, base=base, org=org, proj=proj, use_auth=use_auth
            )
