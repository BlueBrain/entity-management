...
from entity_management_async import nexus, typecheck
from entity_management_async.context import expand, get_resolved_context
from entity_management_async.settings import (
    DASH,
    JSLD_CTX,
    JSLD_ID,
    JSLD_LINK_REV,
...
from entity_management_async.state import get_base_resources, get_base_url, get_org, get_proj
from entity_management_async.util import AttrOf, NotInstantiated, _clean_up_dict, quote
...
async def _deserialize_list(data_type, data_raw, *, context, base, org, proj, token):
    ...
    for data_element in data_raw:
        data = await _deserialize_json_to_datatype(
            list_element_type,
            data_element,
            context=context,
            base=base,
...
async def _deserialize_dict(data_type, data_raw, *, context, base, org, proj, token):
    ...
    return {
        data_key: await _deserialize_json_to_datatype(
...
async def _deserialize_json_to_datatype(
    ...
    try:
        ...
        if issubclass(type_class, Frozen):
            return await _deserialize_frozen(
                data_type, data_raw, context=context, base=base, org=org, proj=proj, token=token
            )
...
async def _deserialize_union(data_type, data_raw, *, context, base, org, proj, token):
    ...
    if typecheck.is_type_single_or_list_union(data_type):
        ...
        return await _deserialize_json_to_datatype(
            type_args[0],
            data_raw,
            context=context,
            base=base,
...
async def _deserialize_identifiable(data_type, data_raw, *, base, org, proj, token):
    ...
    if data_type is Identifiable:
        ...
        if not data_type:
            ...
            data_type = await nexus.get_type_from_id(
...
async def _deserialize_frozen(data_type, data_raw, context, base, org, proj, token):
...
async def _deserialize_resource(
    ...
    if cls == Unconstrained:
        ...
        for field in attr.fields(cls):
            ...
            if field.init and raw is not None:
                type_ = field.type
                init_args[field.name] = await _deserialize_json_to_datatype(
...
class Identifiable(Frozen, metaclass=_IdentifiableMeta):
    ...
    async def from_id(
        ...
        **kwargs,
        ...
        """
        json_ld = await nexus.load_by_id(
        ...
        if json_ld is not None:
            return await _deserialize_resource(
                json_ld,
                cls,
    ...
    @classmethod
    async def from_url(
        cls, url, *, resolve_context=False, base=None, org=None, proj=None, use_auth=None
    ):
        """
        ...
        Args:
            ...
            use_auth (str): OAuth token in case access is restricted.
                Token should be in the format for the authorization header: Bearer VALUE.
        """
        json_ld = await nexus.load_by_url(url, token=use_auth)
    ...
    def __await__(self):  # unasync: remove
        """Allow to await the object itself."""  # unasync: remove
        return self._instantiate().__await__()  # unasync: remove
    ...
    async def _instantiate(self):
        ...
        fetched_instance = await type(self).from_id(
    ...
    async def deprecate(self, sync_index=False, use_auth=None):
        ...
        Args:
            use_auth (str): OAuth token in case access is restricted.
                Token should be in the format for the authorization header: Bearer VALUE.
        """
        await nexus.deprecate(self._self, self._rev, sync_index=sync_index, token=use_auth)
        return self
