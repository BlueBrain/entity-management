...
from entity_management import exception, nexus
...
def test_get_entity__raises_if_not_found(monkeypatch):
    monkeypatch.setattr(nexus, "load_by_id", MagicMock(return_value=None))
    with pytest.raises(
...
def test_get_entity__raises_if_not_instantiated(monkeypatch):
    monkeypatch.setattr(nexus, "load_by_id", MagicMock(return_value={}))
    with pytest.raises(
