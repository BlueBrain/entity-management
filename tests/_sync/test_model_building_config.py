...
def test_download_and_get_ids_from_distribution(monkeypatch):
    monkeypatch.setattr(test_module, "download_file", MagicMock())
    monkeypatch.setattr(
        test_module,
        "file_as_dict",
        MagicMock(return_value={"id": "another_fake_id", "type": "fake_type"}),
    )
...
def test__download_entity_get_ids(monkeypatch, tmp_path):
    mapping = {}
    fake_entity = {"@id": "fake_id", "@type": "fake_type", "_createdAt": "fake_time"}
    monkeypatch.setattr(test_module, "load_by_id", MagicMock(return_value={**fake_entity}))
...
def test__download_entity_get_ids_errors(monkeypatch):
...
def test__download_entity_recursive(monkeypatch):
    mock_download = MagicMock(return_value=["another_fake_id"])
...
def test_download_model_config(monkeypatch, tmp_path):
    mock_download = MagicMock(return_value="fake_mapping")
    mock_write = Mock()
