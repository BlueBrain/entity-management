...
from entity_management_async.settings import NSG
from entity_management_async.state import get_token, has_offline_token, set_token
...
async def test_download_file(tmp_path, httpx_mock):
    ...
    file_path = await nexus.download_file(FILE_URL, path=str(tmp_path))
...
async def test_download_file_with_name(tmp_path, httpx_mock):
    stream = json.dumps(FILE_RESPONSE).encode("utf-8")
    httpx_mock.add_response(
        stream=IteratorStream([stream[: len(stream) // 2], stream[len(stream) // 2 :]]),
        method="GET",
    ...
    new_name = "abc.jpg"
    file_path = await nexus.download_file(FILE_URL, path=str(tmp_path), file_name=new_name)
...
async def test_token():
    ...
    set_token(token)
    assert token == await get_token()
    from entity_management_async.state import ACCESS_TOKEN
...
async def test_load_by_id__cross_bucket():
    resource_id = "https://bbp.epfl.ch/neurosciencegraph/data/1"
    with patch(f"{nexus.__name__}.load_by_url") as patched:
        await nexus.load_by_id(
...
async def test_load_by_id__with_revision():
    resource_id = "https://bbp.epfl.ch/neurosciencegraph/data/1"
...
async def test_load_by_id__with_tag():
    resource_id = "https://bbp.epfl.ch/neurosciencegraph/data/1"
...
async def test_es_query(httpx_mock):
    json_response = {
        "hits": {
            "hits": [
                {
    ...
    res = await nexus.es_query(query, base="https://foo", org="bar", proj="zee")
