...
from entity_management_async import exception, nexus
from entity_management_async import util as test_module
from entity_management_async.base import BlankNode, Frozen
from entity_management_async.core import Entity, attributes
from entity_management_async.typing import MaybeList
from entity_management_async.util import AttrOf
...
async def test_get_entity__raises_if_not_found(monkeypatch):
    monkeypatch.setattr(nexus, "load_by_id", AsyncMock(return_value=None))
    with pytest.raises(
        exception.ResourceNotFoundError, match="Resource id my-id could not be retrieved"
    ):
        await test_module.get_entity("my-id", cls=MyEntity)


async def test_get_entity__raises_if_not_instantiated(monkeypatch):
    monkeypatch.setattr(nexus, "load_by_id", AsyncMock(return_value={}))
    with pytest.raises(
...
async def test_lazy_schema_validator(tmp_path):
    ...
    res = await A(distribution=MockDataDownload()).distribution.as_dict()
    assert res == data
    ...
    with pytest.raises(test_module.SchemaValidationError):
        await B(distribution=MockDataDownload()).distribution.as_dict()
    ...
    with pytest.raises(RuntimeError, match="Expected instance with as_dict method. Got Wrong()"):
        await C(distribution=Wrong()).distribution.as_dict()
