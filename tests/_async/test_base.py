...
from entity_management_async import state
from entity_management_async.base import (
    AttrOf,
    BlankNode,
    BrainLocation,
    Derivation,
...
from entity_management_async.core import ModelRuntimeParameters
from entity_management_async.morphology import ReconstructedPatchedCell
from entity_management_async.settings import JSLD_ID, JSLD_LINK_REV, JSLD_REV, JSLD_TYPE
from entity_management_async.state import (
    get_base_resources,
    get_base_url,
    get_org,
    get_proj,
...
from entity_management_async.typing import MaybeList
from tests.util import TEST_DATA_DIR

state.ACCESS_TOKEN = "foo"


async def test_id_type(monkeypatch):
    ...
    monkeypatch.setattr(
        nexus,
        "create",
        AsyncMock(return_value={JSLD_ID: "id", JSLD_REV: 1, JSLD_TYPE: "Dummy"}),
    )
    ...
    dummy = await dummy.publish()
    assert dummy._id == "id"
    assert dummy._type == "Dummy"


async def test_id_type__nexus_expanded_response(monkeypatch):
    class Dummy(Identifiable):
        """A dummy class"""
    ...
    monkeypatch.setattr(
        nexus,
        "create",
        AsyncMock(return_value={JSLD_ID: "id", JSLD_REV: 1, JSLD_TYPE: "expanded/Dummy"}),
    )
    assert dummy._id is None
    assert dummy._type == "Dummy"
...
async def test_serialize__derivation(monkeypatch):
...
async def test_serialize_obj__include_rev__instantiated_wout_revision(monkeypatch):

    monkeypatch.setattr(
        nexus,
        "load_by_url",
        AsyncMock(return_value={"@id": "foo", "@type": "A", "_rev": 8}),
    )
    ...
    await a._instantiate()  # needed to instantiate _rev from Nexus # unasync: remove
    res = _serialize_obj(a, include_rev=True)
...
async def test_deserialize_json_to_datatype(data_type, data_raw, expected):
    assert await _deserialize_json_to_datatype(_eval(data_type), data_raw) == expected


async def test_deserialize_frozen():
    # test that a frozen deserialized as BlankNode works for backward compatibility
    res = await _deserialize_frozen(BlankNode1, {"a": 2, "b": 3.0}, None, None, None, None, None)
...
async def test_deserialize_json_to_datatype__union(monkeypatch):
    ...
    monkeypatch.setattr(
        nexus,
        "load_by_id",
        AsyncMock(return_value=data_raw_t1),
    )

    res = await _deserialize_json_to_datatype(Union[T1, T2], data_raw_t1)
    await res._instantiate()  # needed because not instantiated from data_raw_t1 # unasync: remove
    ...
    monkeypatch.setattr(
        nexus,
        "load_by_id",
        AsyncMock(return_value=data_raw_t2),
    )

    res = await _deserialize_json_to_datatype(Union[T1, T2], data_raw_t2)
    await res._instantiate()  # needed because not instantiated from data_raw_t2 # unasync: remove
    assert res == T2(c=1, d="2")


async def test_deserialize_json_to_datatype__list_union(monkeypatch):
    ...
    async def mock_load_by_id(resource_id, *args, **kwargs):
    ...
    res = await _deserialize_json_to_datatype(List[Union[T1, T2, T3]], [data_raw_t3, data_raw_t1])
    for item in res:  # unasync: remove
        await item._instantiate()  # needed because not instantiated from data_raw # unasync: remove
...
async def test_unconstrained(monkeypatch, unconstrained_resp):
    monkeypatch.setattr(
        nexus,
        "create",
        AsyncMock(return_value=unconstrained_resp),
    )
    obj = Unconstrained(json=dict(key1="value1", key2="value2"))
    assert get_base_url() == "%s/%s/%s/_" % (get_base_resources(), get_org(), get_proj())
    obj = await obj.publish()
...
async def test_list_by_schema(httpx_mock, cells_page1_resp, cells_page2_resp):
...
async def test_list_by_sparql(httpx_mock):
    httpx_mock.add_response(
    ...
    param = await params.__anext__()
    assert param.get_id().endswith("org/proj/_/fdc9b964-5737-4d58-8d18-cb9af0a1ef38")


async def test_instantiate__wout_rev(monkeypatch):

    async def load_by_id(resource_id, *args, **kwargs):
        if resource_id == "foo":
            return {"_rev": 1}
        raise
    ...
    await r._instantiate()

    # after instantiation the _rev should be 1
    assert r.get_rev() == 1


async def test_instantiate__with_rev(monkeypatch):
...
async def test_instantiate__with_tag(monkeypatch):
    async def load_by_id(resource_id, *args, **kwargs):
...
async def test_instantiate__precedence(monkeypatch):
    async def load_by_id(resource_id, *args, **kwargs):
...
async def test_subject(payload, monkeypatch):
    """Test Subject loading and pyblishing with old and new metadata."""

    async def _mock_load_by_id(resource_id, *args, **kwargs):
    ...
    monkeypatch.setattr(
        nexus,
        "load_by_id",
        AsyncMock(side_effect=_mock_load_by_id),
    )
    res = await Test.from_id("t1-id")
...
async def test_Identifiable_clone(monkeypatch):
