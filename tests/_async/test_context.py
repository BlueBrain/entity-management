...
from entity_management_async import context as test_module
...
async def test_resolve_context(monkeypatch, context_neuroshapes_resp, context_bbp_neuroshapes_resp):
    ...
    res = await test_module._resolve_context(
        [
            "https://bluebrain.github.io/nexus/contexts/metadata.json",
...
async def test_get_resolved_context(
    monkeypatch, context_neuroshapes_resp, context_bbp_neuroshapes_resp
):
    ...
    monkeypatch.setattr(nexus, "load_by_id", mock_load_by_id)
    res = await test_module.get_resolved_context(
        [
            "https://bluebrain.github.io/nexus/contexts/metadata.json",
            "https://bbp.neuroshapes.org",
        ]
...
async def test_get_resolved_context__cycle(
    monkeypatch, context_bbp_neuroshapes_cyclic_resp, context_neuroshapes_resp
):
    async def mock_load_by_id(resource_id, *args, **kwargs):
